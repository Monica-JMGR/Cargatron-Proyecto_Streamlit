...
def home():
    ...
    with st.expander('Descripcion'):
        st.write('Esto es una aplicacion para encontrar el cargador mas cercano a tu casa')

def datos():
    
    path = 'red_recarga_acceso_publico_2021.csv'

    uploaded_file=st.file_uploader('¿Tienes otros datos? ', ['csv'])
    ...
    if df.shape[0] == 0:
        st.warning('No existe ninguna estación con las características seleccionadas')
        st.stop()
...
def filtros(df):
    ...
    select_n_cargadores = st.sidebar.slider('Nº CARGADORES', min_n_cargadores, max_n_cargadores)
    ...
    if checkbox_n_cargadores:
        df = df[df['Nº CARGADORES'] == select_n_cargadores]
